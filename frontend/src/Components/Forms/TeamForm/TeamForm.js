...
const TeamForm = () => {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            name: Yup.string().trim().required('Podaj nazwę drużyny'),
            link: Yup.string()
                .matches(
                    /^https:\/\/wbl.klebiedzinski.pl\/photos\//,
                    'Link musi zaczynac sie od "https://wbl.klebiedzinski.pl/photos/"'
                )
                .required('Podaj link do logo'),
        }),
        onSubmit: (values) => {
    ...
    return (
        <div className={styles.TeamForm}>
            <form onSubmit={formik.handleSubmit}>
                ...
                {error && <h2 className="validation-info">{error}</h2>}
            </form>
        </div>
    );
};
