...
const TeamEditModal = ({ team, setIsModalOpen, id }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      name: Yup.string().required("Podaj nazwę drużyny"),
      link: Yup.string().required(
        "Podaj link do logo, zaczynając od https://wbl.klebiedzinski.pl/photos/teams_pictures"
      ),
      ...
      axiosInstance
        ...
        .then((response) => {
          if (response.status === 200) {
            dispatch({
              type: "UPDATE_TEAM",
              payload: { _id: team._id, name: values.name, logo: values.link },
            });
            setIsEdited(true);
            setTimeout(() => {
              navigate("/teams");
          ...
          <div className={styles.teamEditModal}>
              ...
              >
                ...
                <div className="input-container">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      formik.setFieldValue("teamPicture", e.target.files[0])
                    }
                  />
                  {isSubmitClicked && (
                    <p className="validation-info">
                      {formik.errors.teamPicture}
                    </p>
                  )}
                </div>
                ...
                {isEdited && (
                  <h2 className="validation-info">Edytowano drużynę</h2>
                )}
              </form>
            </div>
            <div className={styles.removeBtn}>
              <img
                src="https://wbl.klebiedzinski.pl/photos/icons/remove-icon.png"
                alt=""
            ...
            {deleteConfirmation && (
              <div className={styles.deleteConfirmation}>
                ...
                <h2>
                  Wraz z nią usuną się wszyscy zawodnicy jak i mecze tej
                  drużyny, cała tabela sie zmieni
                </h2>
                <div className={styles.deleteConfirmationBtns}>
                  <button className="submit-btn" onClick={() => deleteTeam()}>
                    Tak
                  </button>
                  <button
                    className="clear-inputs-btn"
