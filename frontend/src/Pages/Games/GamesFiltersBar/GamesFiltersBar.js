...
const GamesFiltersBar = () => {
    ...
    const handleFilter = () => {
        ...
        const status = (scheduled && finished) ? 'wszystkie' : scheduled ? 'scheduled' : 'finished';
        console.log("status",status)
        axiosInstance.get(`/games/filter/?page=${page}&limit=${limit}&from=${startDate}&to=${endDate}&status=${status}`)
        .then(res => {
            console.log("filtrowane", res.data.games)
    ...
    return (
        ...
        <div className={styles.filtersBar}>
            <button onClick={() => setFiltersOpen(!filtersOpen)}><AiOutlineFilter/></button>
        </div>
