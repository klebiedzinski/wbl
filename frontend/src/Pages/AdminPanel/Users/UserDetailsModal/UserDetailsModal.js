...
const UserDetailsModal = ({ setShowModal, user: user_data }) => {
  ...
  const userTeams =
    teams &&
    teams.teams.filter((team) => user_data.auth_teams.includes(team._id));
  const userPlayers =
    players &&
    players.players.filter((player) =>
      user_data.auth_players.includes(player._id)
    );
          ...
          {teamsLoading || playersLoading ? (
            ...
            <>
              ...
              <div className={styles.controls}>
                {!user_data.adminConfirmed && (
                  <AiOutlineCheck
                    onClick={() => handleAcceptRequest(user_data._id)}
                  />
                )}
                {
                  <AiFillDelete
