...
import { addGameReducer } from "../../../features/games/gamesSlice";
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      status: Yup.string().required(
        "Podaj status meczu (scheduled, live, finished)"
      ),
      team1: Yup.string().required("Podaj nazwę drużyny 1"),
      team2: Yup.string().required("Podaj nazwę drużyny 2"),
      team1Score: Yup.number(),
      team2Score: Yup.number(),
      date: Yup.string().required("Podaj datę meczu (format: YYYY-MM-DD)"),
      location: Yup.string().required("Podaj lokalizację meczu"),
    }),
    onSubmit: (values) => {
      ...
      axiosInstance
        ...
        .then((response) => {
          setIsAdded(true);
          dispatch(addGameReducer(response.data));
        })
        .catch((error) => {
          console.log(error);
        });
  ...
  return (
    <div className={styles.GameForm}>
      <form onSubmit={formik.handleSubmit}>
        ...
        {isAdded && <h2 className="validation-info">Dodano mecz</h2>}
      </form>
    </div>
  );
};
