...
// get games by team id (or all) (scheduled, finished or all)
const getGamesByTeam = async (req, res) => {
  const { id, status } = req.params;
  console.log(id, status);
  if (id === "all") {
    if (status === "scheduled") {
      const games = await Game.find({ status: "scheduled" }).sort({ date: 1 });
      return res.status(200).json({ games });
    } else if (status === "finished") {
      const games = await Game.find({ status: "finished" }).sort({ date: 1 });
      return res.status(200).json({ games });
    } else {
      const games = await Game.find({}).sort({ date: 1 });
      return res.status(200).json({ games });
  ...
  if (!allGames) {
    return res.status(404).json({ error: "Nie znaleziono meczów" });
  }
  if (status === "scheduled") {
    const games = allGames.filter((game) => game.status === "scheduled");
    return res.status(200).json({ games });
  } else if (status === "finished") {
    const games = allGames.filter((game) => game.status === "finished");
    return res.status(200).json({ games });
  } else {
...
const addGame = async (req, res) => {
  const { status, team1_id, team2_id, team1Score, team2Score, date, location } =
    req.body;
    ...
    if (status === "finished") {
      ...
      if (!mongoose.Types.ObjectId.isValid(winner)) {
        return res.status(404).json({
          error: "Nie mogłem dodać zwycięstwa, niepoprawne ID drużyny",
        });
      }
      // grab team by id and update wins, games, points_made and points_lost
      const winningTeam = await Team.findOneAndUpdate(
        { _id: winner },
        {
          $inc: {
            wins: 1,
            games: 1,
            points_made: winnersPoints,
            points_lost: looserPoints,
          },
        }
      );
      if (!winningTeam) {
      ...
      if (!mongoose.Types.ObjectId.isValid(looser)) {
        return res.status(404).json({
          error: "Nie mogłem dodać przegranej, niepoprawne ID drużyny",
        });
      }

      // grab team by id and update losses, games, points_made and points_lost
      const loosingTeam = await Team.findOneAndUpdate(
        { _id: looser },
        {
          $inc: {
            losses: 1,
            games: 1,
            points_made: looserPoints,
            points_lost: winnersPoints,
          },
        }
      );
      if (!loosingTeam) {
...
const updateGame = async (req, res) => {
  ...
  if (req.body.status === "finished" && oldGame.status === "finished") {
    ...
    const winner =
      team1NewScore > team2NewScore ? newGame.team1_id : newGame.team2_id;
    const looser =
      team2NewScore > team1NewScore ? newGame.team1_id : newGame.team2_id;
    const winnersPoints =
      team1NewScore > team2NewScore ? team1NewScore : team2NewScore;
    const looserPoints =
      team1NewScore < team2NewScore ? team1NewScore : team2NewScore;
    const oldWinner =
      team1OldScore > team2OldScore ? oldGame.team1_id : oldGame.team2_id;
    const oldLooser =
      team1OldScore < team2OldScore ? oldGame.team1_id : oldGame.team2_id;
    const oldWinnersPoints =
      team1OldScore > team2OldScore ? team1OldScore : team2OldScore;
    const oldLooserPoints =
      team1OldScore < team2OldScore ? team1OldScore : team2OldScore;

    //REMOVING OLD DATA FROM TEAMS
    const oldWinningTeam = await Team.findOneAndUpdate(
      { _id: oldWinner },
      {
        $inc: {
          wins: -1,
          games: -1,
          points_made: -oldWinnersPoints,
          points_lost: -oldLooserPoints,
        },
      }
    );
    if (!oldWinningTeam) {
    ...
    const oldLoosingTeam = await Team.findOneAndUpdate(
      { _id: oldLooser },
      {
        $inc: {
          losses: -1,
          games: -1,
          points_made: -oldLooserPoints,
          points_lost: -oldWinnersPoints,
        },
      }
    );
    if (!oldLoosingTeam) {
    ...
    if (!mongoose.Types.ObjectId.isValid(winner)) {
      return res
        .status(404)
        .json({ error: "Nie mogłem dodać zwycięstwa, niepoprawne ID drużyny" });
    }
    // grab team by id and update wins, games, points_made and points_lost
    const winningTeam = await Team.findOneAndUpdate(
      { _id: winner },
...
const deleteGame = async (req, res) => {
  ...
  if (game.status === "finished") {
    const winner =
      game.team1Score > game.team2Score ? game.team1_id : game.team2_id;
    const looser =
      game.team2Score > game.team1Score ? game.team1_id : game.team2_id;
    const winnersPoints =
      game.team1Score > game.team2Score ? game.team1Score : game.team2Score;
    const looserPoints =
      game.team1Score < game.team2Score ? game.team1Score : game.team2Score;
