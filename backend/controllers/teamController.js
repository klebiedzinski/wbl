...
const getSingleTeam = async (req, res) => {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
        return res
            .status(404)
            .json({ error: 'Nie mogłem znaleźć drużyny, niepoprawne ID' });
    }
    // find team by id
...
const addTeam = async (req, res) => {
    ...
    if (teamExists) {
        return res
            .status(400)
            .json({ error: 'Drużyna o takiej nazwie już istnieje' });
    }
...
const updateTeam = async (req, res) => {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
        return res
            .status(404)
            .json({
                error: 'Nie mogłem zaktualizować drużyny, niepoprawne ID',
            });
    }
...
const deleteTeam = async (req, res) => {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
        return res
            .status(404)
            .json({
                error: 'Nie mogłem usunąć drużyny, niepoprawne ID drużyny',
            });
    }
    ...
    const games = await Game.deleteMany({
        $or: [{ team1_id: req.params.id }, { team2_id: req.params.id }],
    });
